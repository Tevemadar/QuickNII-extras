...
public class EndianCorrectOutputStream extends DataOutputStream {
     ...
     * @param be -- endian flag: if be (big endian) is false bytes will be flipped
     *           on write
     * @exception FileNotFoundException
     */
    ...
    public long flipLong(long val) {
        ...
        // swap the byte order
        return (long) ((byte0 << 56) | (byte1 << 48) | (byte2 << 40) | (byte3 << 32) | (byte4 << 24) | (byte5 << 16)
                | (byte6 << 8) | byte7);
    }
     ...
     * 
     * double d; float f; int i; short s; byte b; long l;
     * 
     * 
     * 
     * // double 64 try { EndianCorrectInputStream ecs_in = new
     * EndianCorrectInputStream(args[0]+File.separator+"double.dat",
     * Boolean.valueOf(args[1]).booleanValue()); d = ecs_in.readDoubleCorrect();
     * System.out.println("read in "+d); ecs_in.close();
     * 
     * EndianCorrectOutputStream ecs_out = new
     * EndianCorrectOutputStream(args[2]+File.separator+"double.dat",
     * Boolean.valueOf(args[3]).booleanValue()); ecs_out.writeDoubleCorrect(d);
     * ecs_out.close();
     * 
     * } catch (IOException ex) { System.out.println("Oops ! bad double:
     * "+ex.getMessage()); }
     * 
     * // short 16 try { EndianCorrectInputStream ecs_in = new
     * EndianCorrectInputStream(args[0]+File.separator+"short.dat",
     * Boolean.getBoolean(args[1])); s = ecs_in.readShortCorrect();
     * System.out.println("read in "+s); ecs_in.close();
     * 
     * EndianCorrectOutputStream ecs_out = new
     * EndianCorrectOutputStream(args[2]+File.separator+"short.dat",
     * Boolean.getBoolean(args[3])); ecs_out.writeShortCorrect(s); ecs_out.close();
     * 
     * } catch (IOException ex) { System.out.println("Oops ! bad short:
     * "+ex.getMessage()); }
     * 
     * // int32 try { EndianCorrectInputStream ecs_in = new
     * EndianCorrectInputStream(args[0]+File.separator+"int.dat",
     * Boolean.getBoolean(args[1])); i = ecs_in.readIntCorrect();
     * System.out.println("read in "+i); ecs_in.close();
     * 
     * EndianCorrectOutputStream ecs_out = new
     * EndianCorrectOutputStream(args[2]+File.separator+"int.dat",
     * Boolean.getBoolean(args[3])); ecs_out.writeIntCorrect(i); ecs_out.close();
     * 
     * } catch (IOException ex) { System.out.println("Oops ! bad int:
     * "+ex.getMessage()); }
     * 
     * return; }
     *************************************************************/
