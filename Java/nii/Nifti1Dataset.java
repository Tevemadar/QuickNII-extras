 ...
 * <p>
 * Notes: <br>
 * Compression: files compressed with gzip are read using the Java gunzip
 * utility
 *
 * <p>
 * Endian-ness: Java uses big-endian by default. nifti files determined to be
 * little-endian as per the nifti spec (dim[0] field) will have the header and
 * data swapped on read. (Not sure about write yet...)
 *
 * <p>
 * Datatypes: the nifti-1 format supports a large number of datatypes. The
 * current read methods typecast all data to doubles. This API will be extended
 * to include methods to return image blobs of different datatypes.
 *
 *
 * <h3>Todo List</h3>
 * <ul>
 *
 * <li>This API currently uses the traditional Java java.io methods. Will
 * explore performance gains by using the java.nio methods, including possibly
 * memory mapping. I think some of the new java.io methods such as readFully()
 * use nio anyway.</li>
 *
 * <li>For header read and write, should change to read/write fields from/to 348
 * byte blob, and read/write blob from disk in 1 call.</li>
 *
 * <li>Currently the writeHeader() method writes from the lower level variables,
 * not the "shortcut" variables ie pixdim[0] not qfac, dim[1] not XDIM, etc.
 * need to think about this, use accessor methods</li>
 *
 * <li>Will need to change read/writeVol methods to call read/write slice methds
 * b/c blob_size has to be an int but should be a long. 32 bit int only allows
 * max volume 1625^3 which is too small.</li>
 *
 * <li>Add methods to manipulate qform/sform and roatation matrices and
 * quaternion representations</li>
 *
 * <li>Add simple method to make a Java AWT image from a slice</li>
 *
 * <li><br>
 * Possible extensions to this API: Extend this API to be compatible with
 * Analyze7.5 format ?</li>
 * </ul>
 *
 *
 *
 ...
 * <ul>
 * <li>3/1/2005 KF Alpha version put into SourceForge CVS</li>
 * <li>10/2005 KF added copyHeader() routine</li>
 * <li>2/20/2006 KF Bug fix in readHeader() for endian setting, thanks to Jason
 * Dai.</li>
 * <li>3/2006 KF added a little code to readNiiExt() to check if extensions
 * overrun image data.</li>
 * </ul>
 *
 */
...
public class Nifti1Dataset {
    ...
    public String ds_datname; // file name for data
     ...
     * 
     * @param name - name of the nifti-1 dataset. The name can have the .hdr, .img,
     *             or .nii extension, optionally followed by a .gz compression
     *             suffix. Or, the name can be specified with no extensions or
     *             suffixes, in which case the program will look in this order for
     *             files with these extensions: .hdr<.gz> .img<.gz> .nii<.gz>
     */
    ...
    //////////////////////////////////////////////////////////////////
    /**
     * Constructor for creation of a new dataset. Default values are set, programmer
     * must set or reset all fields appropriate for the new dataset.
     *
     */
     ...
     * 
     * @param ecs is an InputStream open and pointing to begining of extensions
     *            array
     * @exception IOException
     */
    private void readNiiExt(EndianCorrectInputStream ecs) throws IOException {
        ...
        try {
            ...
            throw new IOException(
                    "Error: i/o error reading extension bytes on header file " + ds_hdrname + ": " + ex.getMessage());
        }
        ...
        if (extension[0] != (byte) 0) {
            ...
            while (start_addr < (int) vox_offset) {
                try {
                    ...
                    printHeader();
                    throw new EOFException("Error: i/o error reading extension data for extension "
                            + (extensions_list.size() + 1) + " on header file " + ds_hdrname + ": " + ex.getMessage());
                }
                ...
                if (start_addr > (int) vox_offset) {
                    printHeader();
                    throw new IOException("Error: Data  for extension " + (extensions_list.size()) + " on header file "
                            + ds_hdrname + " appears to overrun start of image data.");
                }
     ...
     * 
     * @return nx2 array where n = # of extensions, array elem 0 is the size in
     *         bytes of that extension and array elem 1 is the extension code.
     */
    ...
    public void removeExtension(int index) {
        ...
        if (index >= n) {
            System.out.println("\nERROR: could not remove extension " + index + 1 + " from " + ds_hdrname
                    + ". It only has " + n + " extensions.");
            return;
        }
     ...
     * @param filename -- filename containing the extension. The entire file will be
    ...
    public void addExtension(int code, String filename) throws IOException {
        ...
        if (l > Integer.MAX_VALUE) {
            throw new IOException("Error: maximum extension size is " + Integer.MAX_VALUE + "bytes. " + filename
                    + " is " + l + " bytes.");
        }
        il = (int) l;
        ...
        try {
            ...
            throw new IOException("Error reading extension data for " + ds_hdrname + " from file " + filename + ". :"
                    + ex.getMessage());
        }
        for (i = il; i < il + pad; i++)
            b[i] = 0;
    ...
    public void writeHeader() throws IOException, FileNotFoundException {
        ...
        try {
            ...
            throw new IOException(
                    "Error: unable to write header extensions for file " + ds_hdrname + ": " + ex.getMessage());
        }
    ...
    public void printHeader() {
        ...
        System.out
                .println("Slice timing code:\t\t\t\t" + slice_code + " (" + decodeSliceOrder((short) slice_code) + ")");
        System.out.println(
                "MRI slice ordering (freq, phase, slice index):\t" + freq_dim + " " + phase_dim + " " + slice_dim);
        ...
        System.out.println("Extension bytes 2-4:\t\t\t\t" + (int) extension[1] + " " + (int) extension[2] + " "
                + (int) extension[3]);
    ...
    //////////////////////////////////////////////////////////////////
    /**
     * Check if a valid dataset (header+data) exists. Note that some operations
     * (e.g. header editing) do not actually require that a data file exist. Use
     * existsHdr() existsDat() in those cases. Gzipped files with .gz extension are
     * permitted.
     * 
    ...
    //////////////////////////////////////////////////////////////////
    /*
     * Set fields to make this a nii (n+1) (single file) dataset switching from nii
     * to anz/hdr affects -- magic field "n+1\0" not "ni1\0" -- vox_offset must
     * include 352+extensions -- internal ds_is_nii flag
     *
     * NOTE: all changes are in memory, app still needs to write header and data for
     * change to occur on disk
     *
    ...
    //////////////////////////////////////////////////////////////////
    /*
     * Set fields to make this a ni1 (2 file img/hdr) dataset switching from nii to
     * anz/hdr affects -- magic field "n+1\0" vs "ni1\0" -- vox_offset does
     * notinclude 352+extensions -- internal ds_is_nii flag
     *
     * NOTE: all changes are in memory, app still needs to write header and data for
     * change to occur on disk
     *
     ...
     * 
     * @return datatype code (note: it is not guaranteed to be a valid code, what is
     ...
     * 
     * @param dcode nifti datatype code
     * @return a short with number of bytes per voxel, 0 for unknown, -1 for 1 bit
     */
     ...
     * @return 3D double array, scale and offset have been applied. Array indices
     *         are [Z][Y][X], assuming an xyzt ordering of dimensions. ie indices
     *         are data[dim[3]][dim[2]][dim[1]]
     * @exception IOException
     * 
     */
    ...
    //////////////////////////////////////////////////////////////////
    /**
     * Write one 3D double array to disk. Data is written in datatype of this
     * Nifti1Dataset instance. Data is "un-scaled" as per scale/offset fields of
     * this Nifti1Dataset instance, before being written to disk.
     * 
     * @param data 3D array of array data Array indices are [Z][Y][X], assuming an
     *             xyzt ordering of dimensions. ie indices are
     *             data[dim[3]][dim[2]][dim[1]]
    ...
    //////////////////////////////////////////////////////////////////
    /*
     * Read one 3D volume blob from disk to byte array ttt T dimension of vol to
     * read (0 based index) return byte array
     * 
     * @exception IOException
     */
//	private byte[] readVolBlob(short ttt) throws IOException {
    ...
    //////////////////////////////////////////////////////////////////
    /*
     * Write one 3D volume blob from byte array to disk ByteArrayOutputStream baos
     * -- blob of bytes to write ttt T dimension of vol to write (0 based index)
     * 
     * @exception IOException
     */
//	private void writeVolBlob(ByteArrayOutputStream baos, short ttt) throws IOException {
    public void writeVolBlob(ByteArrayOutputStream baos, short ttt) throws IOException {
        ...
        // System.out.println("\nwriteVolBlob "+ttt+" skip_head: "+skip_head+"
        // skip_data: "+skip_data);
    ...
    //////////////////////////////////////////////////////////////////
    /**
     * Read all the data into one byte array. Note that since the data is handled as
     * bytes, NO byte swapping has been performed. Applications converting from the
     * byte array to int/float datatypes will need to swap bytes if necessary.
    ...
    public byte[] readData() throws IOException {
        ...
        if (blob_size > Integer.MAX_VALUE)
            throw new IOException("\nSorry, cannot yet handle data arrays bigger than " + Integer.MAX_VALUE
                    + " bytes.  " + ds_datname + " has " + blob_size + " bytes.");
    ...
    //////////////////////////////////////////////////////////////////
    /**
     * Write a byte array of data to disk, starting at vox_offset, beginning of
     * image data. It is assumed that usually the entire data array will be written
     * with this call. Note that since the data is handled as bytes, NO byte
     * swapping will be performed. Applications needing byteswapping must swap the
     * bytes correctly in the input array b.
     * 
    ...
    //////////////////////////////////////////////////////////////////
    /**
     * Read one 1D timecourse from a 4D dataset, ie all T values for a given XYZ
     * location. Scaling is applied.
     * 
    ...
    public double[] readDoubleTmcrs(short x, short y, short z) throws IOException {
        ...
        /// System.out.println("\nskip_head: "+skip_head+" skip_data: "+skip_data+"
        /// skip_vol: "+skip_vol);
     ...
     * 
     * @return s String of length n with as much of s as possible, padded with 0 if
     * necessary
     */
